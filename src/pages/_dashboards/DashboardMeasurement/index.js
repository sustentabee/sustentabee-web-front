    ...
    createChart = (
        ...
        if (brand !== null) {
            if (brand.length !== 0) {
                ...
                equipments = equipments.filter(
                    (equipment) => b.indexOf(equipment.brand) !== -1
                );
            }
        }
        ...
        if (startDate !== "") {
            ...
            measurements = measurements.filter((measurement) => {
                const dateMaintenance = moment(
                    new Date(measurement.created_at)
                );
                const duration = moment.duration(
                    dateFilter.diff(dateMaintenance)
                );
        ...
        for (let i = 0; i < equipments.length; i++) {
            let totalConsumo = 0;
            let consumo = measurements.map((measurement) =>
                measurement.name === equipments[i].name
                    ? calcularValorConsumo(
                          calcularConsumo(
                              calcularTempo(
            ...
            let estimatedConsumption = measurements.map((measurement) =>
                measurement.name === equipments[i].name
                    ? calcularConsumo(
                          calcularTempo(
                              measurement.created_at,
                          ...
                          equipments[i].estimatedConsumption,
                          1
                      )
                    : 0
            );
            ...
            arr.push({
                ...
                total: totalConsumo,
                consumoEstimado: estimatedConsumption,
            });
        }
        const totalConsumo = [],
            categories = [],
        ...
        for (let i = 0; i < arr.length; i++) {
            ...
            consumoEstimado.push(arr[i].consumoEstimado);
        }
        this.setState({ totalConsumo, categories, consumoEstimado });
    };
    ...
    render() {
        ...
        const options = {
            ...
            tooltip: {
                ...
                // pointFormat:'<tr><td style="color:series.color;padding:0">series.name: </td><td style="padding:0"><b> R$point.y</b></td></tr>',
                // footerFormat: "</table>",
                shared: true,
                useHTML: true,
            },
            ...
            series: [
                ...
                // { name: "Consumo Estimado", data: consumoEstimado }
            ],
            responsive: {
                rules: [
                    {
                                    ...
                                    <Row className="justify-content-end">
                                        <Col xs={12} lg={6}>
                                            ...
                                            <Select
                                                name="brand"
                                                value={brand}
                                                onChange={this.filterBrand.bind(
                                                    this
                                                )}
                                                options={brands}
                                                isMulti
